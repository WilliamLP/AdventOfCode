WITH RECURSIVE monkey_input AS (
    ...
    SELECT COUNT(*) AS monkey_count FROM monkey_input
), monkey_parsed AS (
    ...
    UNION ALL (
        WITH preprocess AS (
            SELECT next_turn_round, next_turn_monkey, monkey, test_divisor, true_throw, false_throw,
                CASE WHEN next_turn_monkey = monkey THEN
                        (CASE WHEN op = '*' AND arg = 'old' THEN item * item
                            WHEN op = '*' THEN item * arg::INT
                            WHEN op = '+' THEN item + arg::INT END) / 3
        ...
        SELECT CASE WHEN next_turn_monkey + 1 = monkey_count THEN next_turn_round + 1
            ...
            new_item
        FROM preprocess, monkey_count
        WHERE next_turn_round <= 20
    )
), monkey_business AS (
    SELECT next_turn_monkey, COUNT(*) AS processed_count
    FROM monkey_carrying
    WHERE next_turn_monkey = monkey AND next_turn_round <= 20
    GROUP BY 1
)
SELECT (array_agg(processed_count ORDER BY processed_count DESC))[1] *
    (array_agg(processed_count ORDER BY processed_count DESC))[2] AS part1
